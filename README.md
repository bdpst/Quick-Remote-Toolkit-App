...
- настройка доменного пользователя для подготовки SMB-доступа к административным шарам;
...
В блоке **Админ-доступ** можно указать доменного пользователя, например:

```text
GTL\kmn
```

Если поле оставить пустым, приложение подставит текущего пользователя Windows в формате `DOMAIN\User`.
...
Пароль не сохраняется и не передается приложением. Если текущие доменные учетные данные подходят, Windows использует их автоматически.
